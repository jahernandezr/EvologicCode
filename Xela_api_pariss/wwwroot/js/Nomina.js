    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            if (data === 'sininformacion') {
                ...
                $('#datos2').html(
                    '<p>' + data.correo_electronico + '<br>' + data.telefono + '<br> ' + data.sede + ' <br>' + data.direccion + '</p>'
                );
                $('#datos3').html(
                    '<p>' + data.estado + '<br>' + data.cargo + '<br>$ ' + data.valorSueldo + '</p>'
                );
                $('#datos4').html(
                    '<p>Informacion_completa : ' + data.informacion_completa + '<br>Liquidado : ' + data.liquidado + '</p>' 
                );
            //    ModelNMomina1(data);
            }
...
$(document).on('click', '.btnregistrarNuevNom', function () {
    ...
    var dat = { 'Nombre': Nombre, 'Apellido': Apellido, 'Tipo_de_documento': Tipo_de_documento, 'Numero_identificacion': Numero_identificacion, 'Correo_electronico': Correo_electronico, 'Telefono': Telefono, 'Sede': Sede, 'Direccion': Direccion, 'Estado': Estado, 'Cargo': Cargo, 'ValorSueldo': ValorSueldo, 'Liquidado': Liquidado, 'Informacion_completa': Informacion_completa };
    var Param = JSON.stringify(dat);
   // alert(Param);
        $.ajax({
            headers: {
